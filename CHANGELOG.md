...
### Added
- **Quarterly & yearly analytics** — the Analytics tab's **Spend Trend** and **Budget vs Actual** subtabs gain a Month / Quarter / Year toggle (calendar periods). Backed by a new `GET /api/orgs/{org_id}/spend-trend?granularity=` and a `granularity` parameter on `GET /api/orgs/{org_id}/budget-vs-actual`. Three correctness guarantees underpin the roll-ups: (1) **month de-duplication** — when more than one bill exists for the same org-month (e.g. a `dftPDF_…` draft alongside the real `cust_…` bill), only the one with the highest subscriber count is counted, tie-broken by most recent upload, which also removes a latent double-count from the monthly view; (2) **partial buckets are flagged, never annualised** — a quarter or year with fewer than its full complement of months renders with a muted bar and an "N of M months" note; (3) **native or summed budgets** — a bucket uses a period-level budget target (`2026-Q1`, `2026`) when one has been entered, otherwise it sums the monthly targets it contains and reports the coverage (e.g. "summed from 2 of 3 months"). The budget editor and CSV template follow the active granularity, so annual or quarterly targets can be entered directly. Cost-per-Head, Forecast, and Alerts are intentionally unchanged (still monthly). Covered by `tests/test_analytics.py`
...
- **Subscriber history** — `GET /api/orgs/{org_id}/subscribers/{sub_number}/history` returns a number's full cross-bill story: per-period billing rows with deltas, derived lifecycle events (first-seen, name/plan change, cost spike/drop, dormancy gap, reactivation, gone), and the manual change/retag audit trail. Surfaced as a slide-in panel opened from any Registry, Lifecycle, Waste, Trace, or bill drill-down row
- **Trace tab** — a search-first tab to look up any subscriber number across *every* uploaded bill (all organisations) by national significant digits (any format/partial). Returns one instance per bill, sorted by statement date (newest or oldest first), each expandable into that bill's call-record (CDR) detail, plus a chip per matched number that opens the full timeline. Backed by `GET /api/number-search`
